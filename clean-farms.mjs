...
const blockedPattern =
  /송어|양식|낚시|횟집|수산|축산|동물|곤충|목장|카페|펜션|민박|리조트|캠핑|학교|유치원|어린이집|병원|교회|성당|사찰|박물관|휴양림|백화점|아파트|빌라|부동산|공인중개|요양|재활|정육|식당|맛집|입구|주차장|골프|컨트리|호텔|모텔|게스트하우스|스테이|파크|랜드|유원지|놀이|테마파크|동물원|공방|직매장|마켓|유통센터|로컬푸드/;
...
const verifiedCropPattern =
  /토마토|오이|멜론|복숭아|자두|감자|상추|블루베리|마늘|양파|파프리카|수박|딸기|사과|포도|감귤|귤|단감|고구마|시금치|배추|고추|당근|대파|참외|애호박|가지|깻잎|부추|브로콜리|양배추|아스파라거스|체리|키위|대추|쑥갓|청경채|케일|미나리|연근|우엉|매실|오미자|유자|무화과|셀러리|과수원|작목반/;
...
function isBadFarm(farm) {
  ...
  if (farm.source === "Naver Local Search" && !verifiedCropPattern.test(text)) return true;
